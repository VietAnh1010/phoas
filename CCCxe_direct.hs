...
data CC =
  ...
  | CCTakeSubCont Prompt CCF
  | CCPushSubCont V V
  | CCPushSubContDeep Prompt V V
  | CCPair V V
  | CCSplit V CCF2
  | CCInl V
  | CCInr V
...
data R =
    RV V
  | RB Prompt (V -> Frame -> R) CapturedSubcont
...
dispatchCC :: CC -> Frame -> R
dispatchCC (CCAtom v) k = dispatchFrame k v

dispatchCC (CCFun f) k = dispatchFrame k (VFun f)
dispatchCC (CCFix f) k = dispatchFrame k (VFix f)
dispatchCC (CCApp v1 v2) k = case v1 of
  VFun f -> dispatchCCF f v2 k
  VFix f -> dispatchCCF2 f v1 v2 k
  _ -> error "Type error"

dispatchCC (CCPrim1 pr v) k = dispatchFrame k (prim1 pr v)
dispatchCC (CCPrim2 pr v1 v2) k = dispatchFrame k (prim2 pr v1 v2)
dispatchCC (CCBind m f) k = dispatchCC m (F1 f k)

dispatchCC (CCPushPrompt p m) k = resolvePrompt (dispatchCC m F0) p k
dispatchCC (CCTakeSubCont p body) k = RB p (dispatchCCF body) (CSC0 k)

dispatchCC (CCPushSubCont csk v) k = case csk of
  VKont csk -> dispatchCapturedSubcont (appendFrameCSK k csk) v
  _ -> error "Type error"

dispatchCC (CCPushSubContDeep p csk v) k = case csk of
  VKont csk -> resolvePrompt (dispatchCapturedSubcont csk v) p k
  _ -> error "Type error"

dispatchCC (CCPair v1 v2) k = dispatchFrame k (VPair v1 v2)
dispatchCC (CCSplit v f) k = case v of
  VPair v1 v2 -> dispatchCCF2 f v1 v2 k
  _ -> error "Type error"

dispatchCC (CCInl v) k = dispatchFrame k (VInl v)
dispatchCC (CCInr v) k = dispatchFrame k (VInr v)
dispatchCC (CCCase v f1 f2) k = case v of
  VInl v -> dispatchCCF f1 v k
  VInr v -> dispatchCCF f2 v k
  _ -> error "Type error"

dispatchCCF2 :: CCF2 -> V -> V -> Frame -> R
dispatchCCF2 (CCF2 f) v1 v2 = dispatchCC (f v1 v2)

dispatchCCF :: CCF -> V -> Frame -> R
dispatchCCF (CCF f) v = dispatchCC (f v)

dispatchFrame :: Frame -> V -> R
dispatchFrame F0 v = RV v
dispatchFrame (F1 f k) v = dispatchCCF f v k

-- dispatchMetacont :: Metacont -> V -> V
-- dispatchMetacont MC0 v = v
-- dispatchMetacont (MC1 _ k mk) v = dispatchFrame k v mk

resolvePrompt :: R -> Prompt -> Frame -> R
resolvePrompt (RV v) _ k = dispatchFrame k v
resolvePrompt (RB p' body csk) p k = if p == p' then body (VKont csk) k else RB p' body (CSC1 p k csk)

dispatchCapturedSubcont :: CapturedSubcont -> V -> R
dispatchCapturedSubcont (CSC0 k) v = dispatchFrame k v
dispatchCapturedSubcont (CSC1 p k csk) v = resolvePrompt (dispatchCapturedSubcont csk v) p k
...
-- pushSubCont :: CapturedSubcont -> V -> Frame -> R
-- pushSubCont csk v k = pushSubCont' (appendFrameCSK k csk) v

-- -- this is much more elegant
-- pushSubCont' :: CapturedSubcont -> V -> R
-- pushSubCont' (CSC0 k) v = dispatchFrame k v
-- pushSubCont' (CSC1 p k csk) v = pushSubCont' csk v (MC1 p k mk)
...
shift :: ((V -> CC) -> CC) -> CC
shift body = CCTakeSubCont resetPrompt (CCF $ \c -> CCPushPrompt resetPrompt (body $ \v -> CCPushSubContDeep resetPrompt c v))
...
shift0 :: ((V -> CC) -> CC) -> CC
shift0 body = CCTakeSubCont reset0Prompt (CCF $ \c -> body $ \v -> CCPushSubContDeep reset0Prompt c v)
...
control :: ((V -> CC) -> CC) -> CC
control body = CCTakeSubCont promptPrompt (CCF $ \c -> CCPushPrompt promptPrompt (body $ \v -> CCPushSubCont c v))
