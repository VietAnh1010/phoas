...
Definition iheap_alloc (v : val) (h : iheap) : option (loc * iheap) :=
  let (m, l) := h in
  if LocMap.member l m
  then None
  else Some (l, IHeap (LocMap.insert l v m) (loc_succ l)).
...
Definition iheap_write (l : loc) (v : val) (h : iheap) : option iheap :=
  let (m, l') := h in
  if LocMap.member l m
  then Some (IHeap (LocMap.insert l v m) l')
  else None.

Definition iheap_dealloc (l : loc) (h : iheap) : option iheap :=
  let (m, l') := h in
  if LocMap.member l m
  then Some (IHeap (LocMap.remove l m) l')
  else None.
